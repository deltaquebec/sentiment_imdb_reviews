...
with open("results_rnn.txt", "w+") as h:
    print("Results of RNN with cleaned data",file=h)
...
with open("results_rnn.txt", "a+") as h:
...
for i in epochs:
    ...
    # create RNN model with parameters specified above; get training time
    start = datetime.now()
    ...
    # RNN model evaluated on test sets for loss, accuracy, time
...
def time_graph():
    ax=sns.lineplot(data = dr_tr.iloc[0:10]['t'], marker='o').set(xlabel ="epoch", ylabel = "time (s)",title="RNN time per epoch")
    plt.legend(labels=["RNN",],loc = 'lower right')
    plt.savefig("vis_rnn_time.png", dpi=300)
    plt.show()
    return ax
time_graph()
...
def metrics_line():
    ax=sns.lineplot(data = dr_tr.iloc[0:10][['loss','acc','miss']], markers=True).set(xlabel ="epoch", ylabel = "metric value",title="RNN metrics per epoch")
    plt.legend(labels=["loss","acc","miss"],loc = 'right')
    plt.savefig("vis_rnn_metrics_sans_avg.png", dpi=300)
    plt.show()
    return ax
metrics_line()
...
def metrics_bar():
    ax = dr_tr[['loss','acc','miss']].plot.bar(rot=0,title="RNN metrics per epoch")
    ax.set_xlabel("epoch")
    ...
    plt.savefig("vis_rnn_metrics_avec_avg.png", dpi=300)
    plt.show()
    return ax
metrics_bar()
