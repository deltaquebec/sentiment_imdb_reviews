...
# start txt file to store results; print size of training and test data
with open("results_cnn.txt", "w+") as h:
    print("results of cnn with cleaned data",file=h)
...
# save empty arrays for average loss, accuracy, f1, and training time across all epochs
loss = []
acc = []
f1one = []
f1onemic = []
...
for i in epochs:
    ...
    # create CNN model with parameters specified above; get training time
    start = datetime.now()
    ...
    # CNN model evaluated on test sets for loss, accuracy, time
...
def time_graph():
    ax=sns.lineplot(data = dr_tr.iloc[0:10]['t'], marker='o').set(xlabel ="epoch", ylabel = "time (s)",title="CNN time per epoch")
    plt.legend(labels=["CNN",],loc = 'lower right')
    plt.savefig("vis_cnn_time.png", dpi=300)
    plt.show()
    return ax
time_graph()
...
def metrics_line():
    ax=sns.lineplot(data = dr_tr.iloc[0:10][['loss','acc','miss']], markers=True).set(xlabel ="epoch", ylabel = "metric value",title="CNN metrics per epoch")
    plt.legend(labels=["loss","acc","miss"],loc = 'right')
    plt.savefig("vis_cnn_metrics_sans_avg.png", dpi=300)
    plt.show()
    return ax
metrics_line()
...
def metrics_bar():
    ax = dr_tr[['loss','acc','miss']].plot.bar(rot=0,title="CNN metrics per epoch")
    ax.set_xlabel("epoch")
    ...
    plt.savefig("vis_cnn_metrics_avec_avg.png", dpi=300)
    plt.show()
    return ax
metrics_bar()
